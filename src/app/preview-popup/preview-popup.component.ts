...
export class PreviewPopupComponent implements OnInit, OnChanges {
  ...
  getCreator() {
    return (
      this.doc.properties["dc:creator"].id || this.doc.properties["dc:creator"]
    );
  }
  ...
  copyLink() {
    ...
    const extention: string[] = specialExtensions.filter((item: string) =>
      assetName.includes(item)
    );
    ...
    }/asset-view?sector=${sector}&folderStructure=${folderStructure}&extension=${
      extention[0] || "allowed"
    }&assetName=${assetName}`;

...
export class DeviceSettingsComponent implements OnInit {
  ...
  async getOrCreateSupplierFolder() {
    ...
    try {
      const folder = await this.nuxeo.nuxeoClient.repository().fetch(adminPanelWorkspacePath + '/DeviceFolder');
      if (!folder) return this.createFolder("Folder", "DeviceFolder", adminPanelWorkspacePath);
    } catch (err) {
  ...
  async getDeviceList() {
    const url = `/search/pp/nxql_search/execute?currentPage0Index=0&offset=0&pageSize=1000&queryParams=SELECT * FROM Document WHERE ecm:primaryType = 'Device' AND ecm:isVersion = 0 AND ecm:isTrashed = 0`;
  ...
  async getSubAreaList() {
    const url = `/search/pp/nxql_search/execute?currentPage0Index=0&offset=0&pageSize=1000&queryParams=SELECT * FROM Document WHERE ecm:primaryType = 'SubArea' AND ecm:isVersion = 0 AND ecm:isTrashed = 0`;
    const res = await this.apiService
      .get(url, { headers: { "fetch-document": "properties" } }).toPromise();
  ...
  async changeStatus(device, status) {
    ...
    await this.updateDocument(device.uid, {properties: {"device:status": status}});
    this.getDeviceList();
  }
  ...
  filterDevice() {
    this.filteredDeviceList = this.deviceList.filter(device => {
      let match = true;
      if (this.deviceInput) match = match && device.name.toLowerCase().includes(this.deviceInput.toLowerCase());
      if (this.selectedRegions) match = match && device.region.includes(this.selectedRegions.uid);
      if (this.selectedsubAreas) match = match && device.subArea.includes(this.selectedsubAreas.uid);
      if (this.selecteddeviceTypes) match = match && device.deviceTyp.toLowerCase().includes(this.selecteddeviceTypes.toLowerCase());
      if (this.selectedStatus) match = match && device.status.toLowerCase().includes(this.selectedStatus.toLowerCase());
      return match;
    });
  }
